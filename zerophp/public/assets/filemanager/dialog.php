...
// Filter
if(!isset($_SESSION['RF']["filter"])){ $filter=''; $_SESSION['RF']["filter"] = $filter; }
if(isset($_GET['filter'])){ $filter=fix_get_params($_GET['filter']); $_SESSION['RF']["filter"] = $filter; }
...
if(!isset($_SESSION['RF']["filter_date"])){ $_SESSION['RF']["filter_date"] = $filter_date; }
if(isset($_GET['filter_date_type'])){ $filter_date['type']=fix_get_params($_GET['filter_date_type']) == 'lt' ? 'lt' : 'gt'; $_SESSION['RF']["filter_date"] = $filter_date; }
if(isset($_GET['filter_date_value'])){ $filter_date['value']=fix_get_date($_GET['filter_date_value']); $_SESSION['RF']["filter_date"] = $filter_date; }
$filter_date=$_SESSION['RF']["filter_date"];
...
<html xmlns="https://www.w3.org/1999/xhtml">
	...
	<![endif]-->
	<script src="js/jquery.ui.datepicker-vi.min.js" type="text/javascript"></script>
	...
	<?php $protocol = 'http'; ?>
	<input type="hidden" id="current_url" value="<?php echo str_replace(array("&filter=$filter" . '&filter_date_type=' . $filter_date['type'] . '&filter_date_value=' . $filter_date['value']),array(''),$protocol."://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>" />
	    ...
	    <div class="tab-content">
			<div class="tab-pane active" id="tab1">
				...
				<form action="dialog.php" method="post" enctype="multipart/form-data" id="myAwesomeDropzone" class="dropzone">
					...
					<input type="hidden" name="filter_date_type" value="<?php echo $filter_date['type']; ?>"/>
					<input type="hidden" name="filter_date_value" value="<?php echo $filter_date['value']; ?>"/>
...
if ($filter_date['value']) {
    $date = explode('/', $filter_date['value']);
    $date =  $filter_date['type'] == 'lt' ? mktime(23, 59, 59, $date[1], $date[0], $date[2]) : mktime(0, 0, 0, $date[1], $date[0], $date[2]);
    function filter_date_comparison($file_date, $comparision_date, $operator) {
        if ($operator == 'lt') {
            if ($file_date <= $comparision_date) {
                return true;
            }
            else {
                return false;
            ...
            return !filter_date_comparison($file_date, $comparision_date, 'lt');
        }
    }
}
foreach ($files as $file_key => $file_array) {
    ...
    if(    $file == '.'
        || $file == '..'
        || is_dir($current_path.$rfm_subfolder.$subdir.$file)
        || in_array($file, $hidden_files)
        || !in_array(fix_strtolower($file_array['extension']), $ext)
        || ($filter!='' && strpos($file,$filter)===false)
        || ($date && !filter_date_comparison($file_array['date'], $date, $filter_date['type']))) {
...
//PAGER
$pager_limit = 18;
$pager_sum = count($files);
$pager_pages = ceil($pager_sum / $pager_limit);
$pager_current = isset($_GET['page']) ? intval($_GET['page']) : 1;
$pager_current = $pager_current > 1 ? $pager_current : 1;
$pager_current = $pager_current > $pager_pages ? $pager_pages : $pager_current;
$pager_link = '';

function pager_make_links($pager_current, $pager_pages) {
    $pager_link_limit = 2;
    $pager_link_from = $pager_current - $pager_link_limit;
    $pager_link_from = $pager_link_from > 1 ? $pager_link_from : 1;
    $pager_link_to = $pager_current + $pager_link_limit;
    $pager_link_to = $pager_link_to < $pager_pages ? $pager_link_to : $pager_pages;

    $pager_link = '';
    if ($pager_current > 3) {
        $pager_link .= '<a class="pager pager_first" href="javascript:void(\'\')" data-pager="1">Đầu</a>';
    }
    if ($pager_current > 1) {
        $pager_link .= '<a class="pager pager_first" href="javascript:void(\'\')" data-pager="' . ($pager_current - 1) . '">Trước</a>';
    }
    if ($pager_link_from > 1) {
        $pager_link .= '<span class="pager pager_dot pager_unclick">...</span>';
    }

    for ($i = $pager_link_from; $i <= $pager_link_to; $i++) {
        if ($i == $pager_current) {
            $pager_link .= '<span class="pager pager_current pager_unclick">' . $i . '</span>';
        }
        else {
            $pager_link .= '<a class="pager pager_' . $i. '" href="javascript:void(\'\')" data-pager="' . $i . '">' . $i . '</a>';
        }
    }

    if ($pager_link_to < ($pager_pages)) {
        $pager_link .= '<span class="pager pager_dot pager_unclick">...</span>';
    }

    if ($pager_current < $pager_pages) {
        $pager_link .= '<a class="pager pager_first" href="javascript:void(\'\')" data-pager="' . ($pager_current + 1) . '">Tiếp</a>';
    }
    if ($pager_current < ($pager_pages - 2)) {
        $pager_link .= '<a class="pager pager_last" href="javascript:void(\'\')" data-pager="' . $pager_pages . '">Cuối</a>';
    }
...
if ($pager_pages > 1) {
    $pager_link = pager_make_links($pager_current, $pager_pages);
}

$files = array_slice($files, ($pager_current - 1) * $pager_limit, $pager_limit);
			    ...
			    <button class="btn tip<?php if($view==2) echo " btn-inverse"; ?>" id="view2" data-value="2" title="<?php echo lang_View_columns_list; ?>"><i class="icon-fire <?php if($view==2) echo "icon-white"; ?>"></i></button>
			     ...
			     <div class="span7 half types">
    			    ...
    			    <?php } ?>
			         ...
			         <input accesskey="f" type="text" class="filter-input <?php echo (($_GET['type']!=1 && $_GET['type']!=3) ? '' : 'filter-input-notype'); ?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(lang_Text_filter); ?>..." value="<?php echo $filter; ?>" />

			         <select id="filter-date-type" class="filter-input" name="filter_date_type"><option value="gt"<?php echo($filter_date['type'] == 'gt' ? ' selected="selected"' : '') ?>><?php echo lang_Date_Great_Than ?></option><option value="lt"<?php echo($filter_date['type'] == 'lt' ? ' selected="selected"' : '') ?>><?php echo lang_Date_Less_Than ?></option></select>
			         <input id="filter-date-value" class="filter-input" type="text" name="filter_date_value" placeholder="<?php echo fix_strtolower(lang_Date_choose); ?>..." value="<?php echo $filter_date['value']; ?>" />
			         <label id="filter" class="btn filter-btn"><i class="icon-play"></i></label>

			         <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
			         <label id="ff-item-type-all" title="<?php echo lang_All; ?>" <?php if(1==0 && ($_GET['type']==1 || $_GET['type']==3)){ ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn filter-btn btn-inverse ff-label-type-all"><i class="icon-align-justify icon-white"></i></label>
			    </div>
		    </div>
		</div>
	    </div>
...
<?php
    if ($pager_link) {
        echo '<div id="pager">' . $pager_link . '</div>';
    }
?>
<script>
