...
$(document).ready(function(){
	...
	$('input[name=radio-sort]').on('click',function(){
		...
		$('#filter-input').val('');
		$('#filter-date-type').val('');
		$('#filter-date-value').val('');
	...
	$('#filter').on('click',function(){
		...
		var val_type=$('#filter-date-type').val();
		var val_value=$('#filter-date-value').val();
		window.location.href=$('#current_url').val()+"&filter="+val+"&filter_date_type="+val_type+"&filter_date_value="+val_value;
	});
	
	$('#filter-date-value').keypress(function(e) {
		if (e.which == 13) {
			$('#filter').trigger('click');
		}
	});
	
	$("#filter-date-value").datepicker({ dateFormat: "dd/mm/yy" });
	...
	$('.pager:not(.pager_unclick)').on('click',function(){
		_this=$(this);
		if (js_script) {
			$.ajax({
				url: "ajax_calls.php?action=pager&page="+_this.attr('data-pager')
			}).done(function( msg ) {

			});
		}else {
			window.location.href=$('#current_url').val()+"&page="+_this.attr('data-pager');
		}
	});
